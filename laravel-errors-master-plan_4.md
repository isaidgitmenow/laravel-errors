...
**Verificare:** cele 35 de fișiere PHP din acest document au fost extrase și trecute prin `php -l` (PHP 8.4.21), înainte și după ultima rundă de corecții. Afirmațiile despre API-urile Laravel 11.27+/12, Livewire 3, sentry-laravel 4.4+ au fost verificate contra surselor în **patru** runde adversariale (planurile v1→v3 + două runde pe acest document: prima a corectat 15 puncte de logică în cod și 13 inconsistențe, a doua a prins o regresie introdusă de corecții și 15 inconsistențe de text; deciziile eliminate sunt în Anexă D). Clasele pure-PHP (`AttributeReader`, `AttributeScanner`, `Masker`, `DataSanitizer`, `ValidatesErrorInput`, familia `AttributedHttpException`) au fost **executate** contra unor fixture-uri (74 de aserțiuni: `new class`, `Foo :: class`, enum, array auto-referențiat, traversal Windows/POSIX, HMAC, proprietăți promovate). Ce nu s-a putut verifica fără dependențe instalate e marcat explicit cu **⚠ de verificat în workbench**.
...
| WP-03 | Orchestrator | `ErrorManager`, `Contracts/ErrorManagerInterface` (2.0), `Events/*`, `Listeners/MetricsListener` | F-05, F-06, F-18, F-21, F-22a, I-16 | 2.0 / 2.1 / 2.2 |
...
| WP-08 | Comenzi artisan | `Console/Commands/*`, `Console/Concerns/*` | F-03, F-04, F-13, F-14, I-08(cmd), I-13, I-02(`errors:list`), I-17(`audit:prune`), §5(`upgrade-config`) | 2.0 / 2.1 / 2.2 / 3.0 |
...
final class AttributeScanner
{
    ...
    {
        // Domeniul DDD se adaugă aici, la runtime — config/errors.php nu poate depinde de ordinea încărcării config('ddd.*').
    ...
    public function scan(array $paths): array
    {
        ...
        foreach ($this->files($paths) as $file) {
            ...
            // Un atribut invalid pe O clasă nu trebuie să dezactiveze scanarea pentru TOATE (și, prin
            // AttributeCache::load(), inspecția fiecărei excepții). Îl înregistrăm pentru doctor și continuăm.
            try {
...
final class ExceptionInspector
{
    ...
    public static function context(Throwable $e): array
    {
        ...
        foreach ($attrs['with_context'] ?? [] as $property) {
            if (self::readable($origin, $property)) {   // proprietate tipizată neinițializată → Error; o sărim
    ...
    private static function paramValue(Throwable $origin, string $source): mixed
    {
        ...
        if (self::readable($origin, $source)) {
            $v = $origin->{$source};
    ...
    /** Publică ȘI inițializată — `$e->{$prop}` pe o proprietate tipizată neinițializată aruncă Error. */
    private static function readable(object $o, string $property): bool
    {
        if (! property_exists($o, $property)) {
            return false;
        }
        $ref = new \ReflectionProperty($o, $property);

        return $ref->isPublic() && $ref->isInitialized($o);
    }
}
```
...
- Rută API, `debug=false`: clasă mapată → status 402 + mesaj **public**; clasă nemapată → `ApiRenderer`: „Internal Server Error (ref: <ulid>)" (Laravel-ul ar spune „Server Error" doar dacă renderer-ul nostru ar întoarce `null`).
...
**Fișiere:** `src/ErrorManager.php` (rescris), `src/Contracts/ErrorManagerInterface.php` [nou, 2.0], `src/Events/ExceptionReported.php`, `src/Events/ExceptionRendered.php`, `src/Listeners/MetricsListener.php` [noi, 2.1], `src/Support/CriticalLog.php` [nou, 2.0].
...
```php
<?php
// file: src/Contracts/ErrorManagerInterface.php  (2.0 — necesară din prima zi: ErrorHandler și HandlerSlots rezolvă interfața)
...
Evenimentele (2.1) — `readonly` DTO-uri cu exact câmpurile din apelurile de mai sus. `Listeners/MetricsListener.php` (2.1): ascultă `ExceptionReported`, dezactivat implicit (`errors.metrics => null`), apelează închiderea/class-string-ul invokabil din config prin `CallableResolver` — exemplu de integrare Prometheus/StatsD, fără dependență de o bibliotecă. `ExceptionRendered` e emis din `ErrorManager::render()` pentru renderele proprii **și** din `HandlerSlots::runRespond()` cu `renderer: null`, `context: 'laravel'` când răspunsul n-a trecut prin noi **și excepția nu e `pass_through`** (pentru `pass_through` nu se emite niciun eveniment și nu se atinge răspunsul — `assertPassedThrough` din fake se bazează pe înregistrarea din `ErrorManager`, nu pe evenimente) — `HandlerSlots` primește `Dispatcher` în constructor și, ca să nu emită de două ori, sare emisia când răspunsul are deja header-ul intern `X-Laravel-Errors-Rendered` pus de `ErrorManager::render()` (header eliminat tot în `runRespond`, înainte de a ieși).

**Teste WP-03:** `report()` → `true` pentru excepție normală, `false` pentru `#[DontReport]`, `true` cu bypass, `true` pentru `pass_through` (fără reporteri, fără `ExceptionReported`, fără `Context::push`); `isPassThrough()` care aruncă (cache picat) → `CriticalLog`, `report()` întoarce `true`; reporter care aruncă → `Log::critical` o dată, restul pipeline-ului nu e afectat de a doua eroare identică; `isPassThrough` (F-06): `PlainDomainException(previous: ModelNotFound)` → `false`; `ViewException(previous: NotFoundHttpException)` → `true`; `ViewException(previous: Plain)` → `false`; wrapper `AttributedHttpException` → `false` deși `pass_through` conține `HttpException`; `ExceptionReported` cu context sanitizat; listener care aruncă → `critical`, raportarea continuă; `new ErrorManager(config: [...])` din testele existente rămâne valid.
...
```php
<?php
// file: src/Support/MessageResolver.php  (2.0; `ErrorIdentity` și `error_id.in_message` din 2.1 — până atunci fallback-ul e fără „(ref: …)")
 ...
 * Întotdeauna publice, indiferent de mod: #[TranslatedMessage] (scris pentru client) și orice HttpExceptionInterface
 * (abort(503, 'Down') e deliberat; wrapper-ul nostru are deja mesajul public).
 * 'attributed' (implicit): în plus, brut pentru #[HttpCode]; restul generic.
 * 'always': comportamentul pre-2.0 (NU în producție). 'never': generic și pentru #[HttpCode].
...
final class MessageResolver
{
    public static function public(Throwable $e, int $statusCode, array $config = []): string
    {
        ...
        if ($e instanceof HttpExceptionInterface && $e->getMessage() !== '') {   // abort(404) are mesaj gol → fallback
            return $e->getMessage();
        }
...
**`ValidationProblemRenderer` (2.2):** întoarce `null` (→ Laravel: redirect cu erori în sesiune) dacă `! app(HandlerSlots::class)->shouldRenderJson($request, $e)`; onorează întâi `$e->response` (`withResponse()`, `FormRequest::failedValidation`); altfel `type: about:blank`, `title` = textul lui `$e->status` (implicit 422 „Unprocessable Content"), `detail: $e->getMessage()`, `code: VALIDATION_FAILED`, `error_id`, `errors: $e->errors()` — structura Laravel, ca extensie RFC. Înregistrat prin `$exceptions->render(fn (ValidationException …))` — `renderViaCallbacks` alege **prima** închidere înregistrată al cărei tip se potrivește, deci e înregistrat **înaintea** callback-ului generic al pachetului.

**Teste WP-04:** `MessageResolver` în matricea `mode × debug × tip excepție` (inclusiv wrapper → mesaj public în orice mod, `never` × `#[HttpCode]` → generic, `abort(503,'Down', previous: PDOException)` → „Down", `QueryException` → generic cu status text + `(ref: …)`, `getCode()` 404 → „Not Found", nu „Server Error"); Problem Details câmpuri exacte, `about:blank` fără `#[ErrorCode]`, `Content-Type`; class-string invokable apelat, neinvokabil → boot; Filament GET → `null`, cu `X-Livewire` → JSON; Inertia `page`/`flash`/`props` (boot). **Teste existente de rescris:** `RenderersTest:27`, `ThirdPartyRenderersTest:18-26, 23, 25, 50-60, 58, 72, 84` — ca perechi debug on/off.
...
```php
<?php
// file: src/Reporters/LogReporter.php  — stare finală 2.2 (corectat în 2.0 și, tot în 2.0, scos din `reporters` default; `error_id` din 2.1, `error_code` din 2.2). Rămâne pentru #[ReportTo].
...
```php
<?php
// file: src/Support/DataSanitizer.php  (2.0; în 2.0 `Masker::REDACTED` e o constantă locală `'[REDACTED]'`, mutată în `Masker` la 2.1)
...
**Fișiere:** `src/Console/Concerns/ValidatesErrorInput.php` [nou], `src/Console/Concerns/BuildsErrorStubs.php` (rescris), `src/Console/Commands/{MakeExceptionCommand,MakeDddErrorCommand}.php` (rescrise), `src/Console/Commands/{CacheErrorsCommand,ClearErrorsCacheCommand,DoctorCommand,ListErrorsCommand}.php` [noi], `src/Console/Commands/{PruneAuditCommand,UpgradeConfigCommand}.php` [noi, 3.0], `src/Support/{DetectorProbe,HandlerSlotInspector}.php` [noi].
...
2.2  COMPUNERE (un breaking declarat: Context)
                                   ...
                                   WP-06 Sentry processor, Flare · WP-02 RateLimitKey
...
return [
    ...
    // Ce ajunge la client
    'expose_messages'         => 'attributed',   // attributed | always (NU în producție) | never — #[TranslatedMessage] și HttpExceptionInterface rămân publice în orice mod
    ...
    'metrics'          => null,   // Closure | class-string invokable | null — primește ExceptionReported (MetricsListener)
    'web_renderer'     => ['prefer_laravel_views' => true],   // errors::{status} al Laravel înaintea view-ului pachetului
...
**2.0 — noi:** `Contracts/ErrorManagerInterface.php` · `Support/MessageResolver.php` · `Support/CallableResolver.php` · `Support/CriticalLog.php` · `Console/Concerns/ValidatesErrorInput.php` · `Exceptions/InvalidConfigurationException.php` · `tests/Regression/*` (Anexă C)
...
**2.1 — modificate:** `Attributes/{TranslatedMessage,WithContext}.php` · `ExceptionInspector.php` (prin cache) · `ErrorHandler.php` (slot respond) · `Facades/LaravelErrors.php` · `Support/{MessageResolver,DataSanitizer}.php` (`ErrorIdentity`, `Masker`) · `Reporters/LogReporter.php` (`error_id`)

**2.2 — noi:** `Attributes/{ErrorCode,LogAs}.php` · `Exceptions/AttributedHttpException.php` + 9 sub-clase · `Support/{AttributedExceptionMapper,RateLimitKey,ProblemDetailsFormatter}.php` · `Renderers/ValidationProblemRenderer.php` · `Console/Commands/ListErrorsCommand.php` · `Testing/ErrorManagerFake.php` · `Testing/Concerns/InteractsWithErrors.php` · `Integrations/Sentry/AttributesEventProcessor.php` · `Integrations/Flare/AttributesContextProvider.php` · `docs/{testing,integrations}.md`
...
| `#[RetryAfter(seconds)]` | clasă | ≥ 1 | header `Retry-After` pus de `HandlerSlots::runRespond()` pe toate căile (+ în wrapper pentru consistență) — 3.0 |
...
| `ProductionMessageLeakTest` | F-02: `QueryException`, `debug=false`, API → începe cu „Internal Server Error" (+ `(ref: …)`), fără SQL |
...
| `UnknownLogChannelWarnsTest` | F-12: `#[ReportTo('typo')]` → linie pe canalul default cu `laravel_errors_warning` |
| `NamespacePrefixPathTest` | F-13: `make:error AppPayments/AppFailed` → `app/Exceptions/AppPayments/AppFailed.php` |
| `InvalidHttpOptionTest` | F-14: `--http=200` → exit 1, fără fișier |
| `ApiPrefixDecisionTest` | F-17: `api_prefixes => ['v2']` → `/v2/x` e API; `LaravelErrors::renderJsonWhen()` influențează `ApiDetector` |
| `InvokableFormatterTest` | F-24: class-string invokabil apelat; neinvokabil → `InvalidConfigurationException` la boot |
| `TranslationGroupKeyTest` | F-28: `#[TranslatedMessage('errors')]` (grup) → `null`, fără `TypeError` |
| `McpHardeningTest` | F-09/F-10/F-19: `tail(0)`, UTF-8 invalid, linie 1 MB, notificare cu metodă necunoscută |
| `InertiaPropsRemovedAtBootTest` | F-07/WP-11: `inertia_mode => 'props'` → `InvalidConfigurationException` la boot |
...
| Închiderea `context()` fără `try/catch` | `Handler::exceptionContext()` nu protejează callback-urile → o eroare oprea logarea oricărei excepții (găsit la analiza implementării `121c1b6`) |
| `$origin->{$property}` fără `isInitialized()` | proprietate tipizată neinițializată → `Error` în calea de log |
| `HttpExceptionInterface` cu mesaj gol întors verbatim | `abort(404)` devenea `"message": ""` |
| `method_exists` pentru `report()` pe original | `report()` protected → `container->call()` arunca în `reportThrowable` |
| `AttributeScanner::scan()` fără `try/catch` per clasă | un atribut invalid dezactiva scanarea și inspecția pentru toate clasele |
